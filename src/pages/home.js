export function generateChatPage() {
    ...
    function resetUI() {
        const els = {
            'join-ui': document.getElementById('join-ui'),
            'room-ui': document.getElementById('room-ui'),
            ...
            'room-key': document.getElementById('room-key')
        };
        ...
        // 重置状态
        roomId = ''; username = ''; joined = false; roomPassword = '';
        cleanupEnabled = true; // 重置倒计时开关
        
        // 恢复界面
        els['join-ui'].style.display = 'flex';
        els['room-ui'].style.display = 'none';
        els['chat'].innerHTML = '<div style="text-align:center;color:#999;margin-top:50px"><i class="fas fa-shield-alt fa-3x"></i><br><br>请输入房间ID进入聊天</div>';
        els['userlist'].innerHTML = '';
        els['message'].disabled = true; els['message'].value = '';
        els['send'].disabled = true;
        els['join'].style.display = 'inline-block';
        els['username'].disabled = false;
        els['room-id'].value = ''; els['room-key'].value = '';
    }
