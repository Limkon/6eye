...
const fs =fs.readFileSync(MASTER_SECRET_KEY_FILE, 'utf8').trim();
...
            'auth=1; Max-Age=3600; HttpOnly; Path=/; SameSite=Lax', // Expires in 1 hour
            'is_master=true; Max-Age=3600; HttpOnly; Path=/; SameSite=Lax' // Expires in 1 hour
        ]);
        console.log("[AUTH_GATE] 主密码设置成功。自动登录为主用户并重定向到管理页面。");
...
        } else if (res && !res.writableEnded) { // Ensure response is ended if possible
